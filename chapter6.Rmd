...
```
Clearly, the groups have level differences in terms of weight
...
```
The graph suggest that there are statistically significant differences in weight by group 1 and the others, but it is hard to say if there are differences between group 2 and 3, because the confdence intervals overlap. However, this graph does not tell if the diet is the reason we see the level differences, because the level differences were there already at day 1.
...
ggplot(RATSL8S, aes(x = Group, y = mean)) +
  ...
  theme_minimal()
```
Graph shows that there are outliers, let us get rid off them.
...
```
We can't conclude that there are statistically significant differences between groups, since the p-value of F-test is bigger than the critical 0.05. In other words, we can not reject the (null) hypothesis that groups means are equal to each other.
...
Coefficient of treatment 2 is positivie meaning that the score of bprs is higher in on average in treatment 2, but that is not statistically significant. Linear models assumes independence of obs, which is not realistics here because we have panel data. Let us use random effects.
...
```
Still the coefficient of treatment2 is not significant. Let see what happend we not only allow different intercepts for individuals but also different slopes.
...
The coefficient of treatment2 is still not significant but running the anova test we can conlcude that the fit is better when we include random intercept and slope. Let's try to do even better job by including interaction terms.
...
Looks like anova gives p-value of 0.07 for chisquare meaning that there is no statistically significant difference between the models.
